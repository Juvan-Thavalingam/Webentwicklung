...
const oldAndLiving = function (arr){
    let y = 0
    const resultArr = [];
    let newArr = [...arr]
    for (let x = 0; x < newArr.length; x++){
        if (newArr[x].year < 0 && newArr[x].living){
            resultArr[y++] = newArr[x].name
        }
    }
    return resultArr
}

const numberOfCodes = function (n){
    var arr = SCRIPTS[n].ranges
    let y = 0
...
const oldAndLiving = (arr) => {
    return arr.filter(item => item.year < 0 && item.living)
        .map(item => item.name);
};

//console.log(oldAndLiving(SCRIPTS))
//console.log(numberOfCodes(3))
